...
function ncovCountGroup({top, left, width, height, diffPrefix, diffCount, count, color, name}) {
  ...
  const {width: diffWidth} = json2canvas.utils.getTextWidth({text: diffPrefix + diffCount, fontSize: DIFF_FONT_SIZE});
  const {width: prefixWdith} = json2canvas.utils.getTextWidth({text: diffPrefix, fontSize: DIFF_FONT_SIZE});
...
function newsBlock({title, p}, {top, left}) {
  ...
  const lines = json2canvas.utils.getTextWidth({width: 280, fontSize: 14, fontWeight: 400, lineHeight: 16, text: title, top: singleHeight, left: 20});
